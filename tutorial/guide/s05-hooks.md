...
| `Setup` | 初始化阶段 | 环境检测 |
| `StopFailure` | agent 停止失败后 | 错误恢复、重试逻辑 |
| `TeammateIdle` | 团队成员空闲时 | 多 agent 协作调度 |
| `TaskCreated` | 任务创建时 | 任务跟踪、通知 |
| `TaskCompleted` | 任务完成时 | 后续流程触发 |
| `ConfigChange` | 配置变更时 | 配置热重载、审计 |
| `InstructionsLoaded` | 指令加载后 | 指令验证、增强 |
...
> **执行顺序保证**：同一事件的多个 hook 按来源优先级串行执行（userSettings -> projectSettings -> localSettings -> policySettings）。Hook 之间不能传递状态，每个独立执行。
...
> :warning: **成本与风险**：Agent hook 每次触发都消耗 API tokens，高频事件（如 Stop）可能导致显著成本。Agent hook 运行在 `dontAsk` 模式下，prompt 被注入可能导致意外行为。
...
LLM 领域已知的自我一致性偏差（self-consistency bias）[R1-5] 直接解释了 Hooks 的必要性：模型评估自己几乎总是批准。Hooks 提供了一个**外部验证通道**——让非模型的逻辑（linter、测试、安全扫描）介入 agent 的执行循环，打破自评估的闭环。
