...
核心发现：优秀的压缩策略能实现 **98.7% 的 token 缩减**，同时保持任务完成质量 [R2-10]。这验证了 Claude Code 三层压缩架构的设计选择。
...
静态 leaderboard 上的 1% 差异无法检测模型在 50 步之后是否偏离轨道。**推理能力在约 3,000 token 处开始衰减，prompt 的实际甜蜜点在 150-300 词** [R1-6]。这意味着 compress 和 isolate 策略不是"有更好"，而是**结构性必需**。
