...
> "Forget wisely, remember what matters" · 预计阅读 18 分钟

**核心洞察：微压缩用零成本裁剪把自动压缩的触发频率降低了 80%——最好的 API 调用是不需要发出的那一次。**
...
![三层压缩策略对比](/diagrams/s07-compression-layers.png)
...
> **注意**：`COMPACTABLE_TOOLS` 列表中的 `Bash` 实际上来自 `SHELL_TOOL_NAMES`，它可能包含多个 shell 工具（不仅是 Bash），例如在不同平台或配置下可能存在其他 shell 工具变体。
...
**防工具调用设计**：压缩 prompt 前置了 `NO_TOOLS_PREAMBLE`，明确禁止模型在摘要任务中调用工具（`prompt.ts:19-26`）。这是针对 Sonnet 4.6+ 在 `maxTurns: 1` 下仍尝试调用工具的问题（2.79% vs 0.01% 的差异）。
...
`src/services/compact/apiMicrocompact.ts` 中的 `getAPIContextManagement()` 实现了服务端压缩策略，利用 Anthropic API 的原生 `context_management` 能力在服务端直接清除旧 tool_result。这与客户端微压缩互补——客户端在本地替换旧结果为占位符，API 层则在服务端做更激进的清理。
...
# Layer 1: 微压缩 — 零成本，每轮自动
def micro_compact(messages):
    """把旧的 tool_result 替换为占位符，保留最近 N 个"""
    for msg in messages[:-KEEP_RECENT]:
        if msg.role == "tool_result" and msg.tool in COMPACTABLE_TOOLS:
            msg.content = "[Old tool result cleared]"  # 零 API 调用

# Layer 2: 自动压缩 — token 超阈值时触发
def auto_compact(messages, token_count, threshold):
    """用 LLM 将旧消息摘要为简短总结"""
    if token_count < threshold:
        return  # 未触发
    old, recent = split_at_boundary(messages)
    summary = llm_summarize(old)           # 1 次 API 调用
    return [summary_message] + recent      # 摘要 + 最近消息

# Layer 3: 手动压缩 — 用户 /compact 触发
def manual_compact(messages, instructions=""):
    """同 auto_compact，但用户可附加压缩指令"""
    summary = llm_summarize(messages, extra=instructions)
    save_transcript(messages)              # 保留原始对话到 .transcripts/
    return [summary_message]
```

完整参考实现（含边界标记、转录持久化、上下文恢复）：

<details>
<summary>展开查看完整 Python 伪代码（641 行）</summary>
...
| 清除占位符 | `src/services/compact/microCompact.ts:36` | `TIME_BASED_MC_CLEARED_MESSAGE = '[Old tool result content cleared]'` |
...
|------|------------|--------|-------|
| 压缩层数 | 三层（微压缩+自动+手动） | 两层（`/summarize`+自动） | 单层（repo-map+截断） |
| 零成本压缩 | ✅ 微压缩替换旧 tool_result | ❌ | ✅ repo-map 静态索引 |
| 缓存感知 | ✅ cached micro compact | ❌ | ❌ |
| 压缩后恢复 | ✅ 文件/Skill/Plan 完整恢复 | 部分 | 无 |
...
### 未来演进：Context Collapse 与 Reactive Compact

源码中有两个实验方向：

- **Context Collapse**（`autoCompact.ts:215-223`）：当启用时完全抑制 autocompact，因为 Collapse 自己管理上下文压力（90% 触发 commit、95% 触发 blocking）。源码注释："Collapse IS the context management system when it's on"。
- **Reactive Compact**（`autoCompact.ts:192-199`）：抑制主动 autocompact，改为在 API 返回 `prompt_too_long` 时才被动触发。这涉及主动压缩（可能过早丢失上下文）vs 被动压缩（保留更多上下文但有报错风险）的权衡。

## Why：设计决策与行业上下文

### Factory AI 36K+ 生产消息评估：结构化摘要最优

Factory AI 开发了目前最系统的上下文压缩评估框架，覆盖了 **36,000+** 条来自调试、代码审查和功能开发的生产消息 [R2-9]。他们测试了三种策略：

1. **截断法**：简单丢弃早期消息——快速但信息损失不可控
2. **基于嵌入的压缩**：使用 API 压缩——中等质量
3. **结构化摘要**：保留关键决策和上下文——**效果最好**

核心发现：结构化摘要显著优于截断和嵌入压缩。Factory AI 在另一份报告中指出，优秀的压缩策略能实现高达 **98.7% 的 token 缩减**（如将 150K token 压缩至 2K），同时保持任务完成质量 [R2-10]。这验证了 Claude Code 三层压缩架构的设计选择。

### Manus 的 KV-cache 优先策略

Manus AI 提出了一个重要观点：**KV-cache 命中率是比 token 数更重要的生产指标** [R2-11]。他们的关键实践：

- **工具遮蔽而非移除**：不改变工具列表以保持缓存稳定 [R2-12]
- **文件系统作为上下文管理器**：将长输出写入文件而非直接塞入对话

这与 Claude Code 的 system prompt 分段缓存设计（s03）和微压缩策略高度一致——**改变越少，缓存命中越高**。

### Durability 衰减：为什么 compact 是结构性必需

Philschmid 指出了一个被 benchmark 忽视的指标——**durability**（持久性）[R1-1]：

> "A 1% difference on a leaderboard cannot detect the reliability if a model drifts off-track after fifty steps."

静态 leaderboard 上的 1% 差异无法检测模型在 50 步之后是否偏离轨道。研究表明推理能力随上下文长度增加而逐步衰减，且 prompt 的实际甜蜜点往往远短于上下文窗口上限 [R1-6]。这意味着 compress 和 isolate 策略不是"有更好"，而是**结构性必需**。

### 自治压缩中间件模式

2026 年出现的最佳实践是**自治压缩中间件**：Agent 自己决定何时压缩，配合 85% 上下文使用率的安全回退阈值 [R2-中间件]。Claude Code 的 autoCompact 正是这一模式的实现——不依赖固定规则，而是让系统在接近上限时自主决策。

> **参考来源：** Factory AI/ZenML [R2-9][R2-10]、Manus [R2-11][R2-12]、Philschmid [R1-1]。完整引用见 `docs/research/05-harness-trends-deep-20260401.md` 和 `docs/research/06-agent-architecture-deep-20260401.md`。
...
**调试压缩行为**：可用 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 环境变量降低阈值来测试自动压缩触发，查看 `.transcripts/` 中的转录文件理解压缩前后差异。
...
- [The Unreasonable Effectiveness of an LLM Agent Loop](https://sketch.dev/blog/agent-loop) — 核心循环为什么需要压缩策略支撑
- [Effective harnesses for long-running agents](https://www.anthropic.com/engineering/harness-design-for-coding-agents) — Anthropic 官方对长任务 agent 的 harness 设计指导
...
<!--@include: ./_fragments/sim-s07.md-->

## 可视化

<!--@include: ./_fragments/viz-s07.md-->

## 设计决策

<!--@include: ./_fragments/ann-s07.md-->
