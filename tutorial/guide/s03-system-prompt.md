...
> "Every token in the prompt has a price tag" · 预计阅读 20 分钟

**核心洞察：系统提示词的排列顺序决定缓存命中率——把不变的放前面，变化的放后面，这一个决策省下 90% 的 token 成本。**
...
系统提示词组装的核心是四层拼接 + 缓存边界切分：

```python
# 系统提示词组装（精简版）
def build_system_prompt():
    blocks = []
    # Layer 1: 静态前缀（永远不变，缓存命中率最高）
    blocks.append({"text": IDENTITY_PROMPT + TOOL_INSTRUCTIONS})
    
    # Layer 2: 工具描述（工具集稳定时可缓存）
    blocks.append({"text": format_tools(enabled_tools)})
    
    # ---- cache boundary ---- 以上最大化缓存复用
    blocks.append({"text": DYNAMIC_BOUNDARY, "cache_control": {"type": "ephemeral"}})
    
    # Layer 3: 动态上下文（每次变化：git status, CLAUDE.md, 时间...）
    blocks.append({"text": get_environment_context()})
    
    # Layer 4: 用户配置（放在消息列表中，不放 system prompt）
    # → 这样 system prompt 的缓存可以跨用户共享
    return blocks
```

完整参考实现（含四种缓存模式、breakpoint 策略、token 估算）：

<details>
<summary>展开查看完整 Python 伪代码（465 行）</summary>
...
### 推理模型上 "Think step by step" 反而有害

2026 年出现了一个重要分化 [R1-6]：推理模型（GPT-5、Claude 4.6 with extended thinking）不需要显式推理引导，"think step by step" 等指令反而有害。ALL-CAPS、"YOU MUST"、"NEVER EVER" 等激进格式会过度触发 Claude，产生更差的结果。

这对 system prompt 设计有直接影响：prompt 需要区分"引导推理"和"提供上下文"两种目的。Claude Code 的 system prompt 避免了传统的 chain-of-thought 技巧，专注于提供上下文信息。

### KV-cache 命中率是最重要的生产指标

Manus AI 发现 KV-cache 命中率比 token 数更重要 [R2-11]。他们的关键实践是**工具遮蔽而非移除**——不改变工具列表以保持 KV-cache 前缀稳定 [R2-12]。

Claude Code 的 system prompt 分段设计（静态段在前、动态段在后）正是为此优化：静态段不变 = 缓存命中，动态段变化不影响静态段缓存。这是当前 AI 应用层最大的工程差异化。

### 从 Prompt Engineering 到 Context Engineering

行业术语已从 "Prompt Engineering" 演进为 "Context Engineering" [R1-6]。核心转变：瓶颈不再是"你问什么"，而是"问题周围提供了什么信息"。LangChain 的四策略框架（Write/Select/Compress/Isolate）[R1-3] 加上 2026 年新增的两个策略（Progressive Disclosure/Routing）[R1-11][R1-12]，构成了完整的上下文工程方法论。

> **参考来源：** The AI Corner [R1-6]、Manus [R2-11][R2-12]、LangChain [R1-3]。完整引用见 `docs/research/` 目录下的调研报告。
