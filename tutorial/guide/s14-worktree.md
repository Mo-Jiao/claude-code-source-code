...
**Git Worktree 60 秒速成**：`git worktree` 允许在同一仓库中同时检出多个分支到不同目录，共享 `.git` 数据（无需 clone），创建毫秒级完成。`git worktree add ../feature-branch feature` 创建，`git worktree remove ../feature-branch` 删除。
...
### 大仓库优化：Sparse Checkout

`WorktreeSession` 类型包含 `usedSparsePaths?: boolean` 字段。对于 1GB+ 的 monorepo，sparse-checkout 只检出需要的子目录，显著减少创建时间和磁盘占用。

### 复制 Gitignored 文件

`.worktreeinclude` 文件指定哪些 gitignored 文件应被复制到 worktree（如 `.env.local`、编译缓存）。很多项目的运行依赖这些文件，不复制则 worktree 中代码无法正常运行。
...
<!--@include: ./_fragments/sim-s14.md-->

## 设计决策

<!--@include: ./_fragments/ann-s14.md-->
