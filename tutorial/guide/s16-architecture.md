...
理解哪些代码在每次循环中执行（热路径），哪些只在特定条件下触发（冷路径），有助于把握性能优化的优先级：

> **热路径 vs 冷路径**：
> - **热路径**（每次循环执行）：callModel → 流式接收 → 工具执行 → 追加消息
> - **冷路径**（条件触发）：auto-compact（token 超限时）、Skill 匹配（首次输入时）、Memory 检索（按需）
>
> 性能优化应优先聚焦热路径。例如，Prompt Cache 的命中率直接影响热路径中 callModel 的延迟和成本；而 auto-compact 虽然耗时较长，但触发频率低，对整体体验的影响相对可控。
...
| REPL 启动器 | `src/replLauncher.tsx` | launchRepl() — Ink 渲染入口 |
...
Claude Code 的系统提示词不是一个静态字符串，而是一个精心设计的六层结构（s03）。最关键的设计是：**将稳定内容放在前面，动态内容放在后面**，配合 Anthropic API 的 `cache_control: ephemeral` 断点，实现接近 100% 的 prompt cache 命中率。
...
> **Prompt Cache 失效的常见场景**：
> - 工具列表变化（MCP 服务器重连后工具集改变）
> - CLAUDE.md 修改
> - 模型切换
> - 系统提示词中的动态内容变化（如 git status 更新）
>
> 设计 agent 时需有意识地将高频变化的内容放在 cache 断点之后。Claude Code 的六层提示词结构正是基于这个原则设计的——稳定层在前（命中缓存），动态层在后（不影响缓存）。
...
- **子 Agent**（s12）：复杂任务分派给独立的子对话，完成后只返回摘要结果
...
Claude Code 内置约 10 个核心工具，但通过延迟加载机制（s09），在系统提示词中只发送工具名和搜索提示，不发送完整描述。当模型需要某个工具时，才加载完整定义。
...
系统提示词的六层结构（s03）不仅是内容组织——它是缓存优化和项目感知的统一方案：
...
### 完整对比表：Claude Code vs OpenCode vs Cursor vs Codex CLI vs Windsurf

| 维度 | Claude Code | OpenCode | Cursor | Codex CLI | Windsurf/Cascade |
|------|------------|----------|--------|-----------|-----------------|
| **核心架构** | Agent Loop + QueryEngine | Agent Loop（Go） | IDE 内嵌 LLM | Agent Loop + sandbox | IDE + "flow action" |
| **UI 框架** | React/Ink（终端） | Bubble Tea（终端） | Electron（GUI） | 终端 | Electron（GUI） |
| **状态管理** | Zustand 风格 store | Go struct | Redux-like | 最小化状态 | 自定义状态 |
| **工具数量** | ~10 内置 + MCP 延迟加载 | ~8 内置 | ~15 内置 | ~5 内置 | ~15 内置 |
| **工具加载** | 延迟加载 + 搜索提示 | 全量加载 | 全量加载 | 全量加载 | 全量加载 |
| **搜索策略** | 主动推理（工具调用） | 主动推理（工具调用） | RAG + 主动推理混合 | 主动推理 | RAG + 主动推理 |
| **上下文压缩** | AI 摘要 Compact | 无（依赖窗口大小） | 滑动窗口 | 无 | 滑动窗口 |
| **子 Agent** | 独立对话隔离 | 无 | 无 | 无 | 无 |
| **安全模型** | 六级权限瀑布 | 基础确认 | 两级（信任/不信任） | OS 层 sandbox（默认拒绝网络、只读文件挂载） | 基础确认 |
| **缓存优化** | 层级 prompt + cache_control | 无 | 部分 | 无 | 部分 |
| **记忆系统** | CLAUDE.md + Auto Memory | Rules 文件 | .cursorrules | 无 | .windsurfrules |
| **MCP 支持** | 六层配置 + 企业策略 | stdio/sse | stdio/sse | 无 | stdio/sse |
| **Hooks** | PreToolUse/PostToolUse/Notification | 无 | 无 | 无 | 无 |
| **双模式** | REPL + SDK | CLI only | GUI only | CLI only | GUI only |
| **企业管理** | managed settings + 策略过滤 | 无 | 基础团队管理 | 无 | 基础团队管理 |
| **启动优化** | 并行预取 + fast-path 分发 | 快（Go 编译） | 中（Electron 启动） | 快（Rust 编译） | 中（Electron 启动） |
| **开源状态** | 源码可读（编译产物） | 完全开源 | 闭源 | 开源 | 闭源 |
| **特色设计** | — | — | — | OS sandbox 安全隔离 | "flow action"将多个工具调用合并为单个语义操作 |
...
- **Codex CLI** 的优势在安全隔离——OS 层 sandbox 提供最刚性的安全保证，适合无人值守的自动化场景
- **Windsurf/Cascade** 的差异化在于"flow action"设计——将多个细粒度工具调用合并为单个语义操作（如"重构这个函数"对应一组搜索+读取+编辑），减少用户感知的操作步骤
...
| s03 System Prompt | 系统提示词六层结构 + 缓存优化 | 大脑皮层：行为指令中心 |
| s04 Permissions | 六级权限瀑布 + 自动分类器 | 免疫系统：安全防线 |
...
| s09 Skills | 两层知识注入 + 延迟加载 | 节能：按需激活能力 |
| s10 Plan Mode | 权限切换 + 约束规划 | 前额叶：先想后做 |
| s11 Tasks | 文件型 DAG + 依赖追踪 | 任务调度：进度管理 |
| s12 Sub-Agent | 独立对话 + 上下文隔离 | 分身：并行任务处理 |
| s13 Teams | 文件邮箱 + 生命周期协议 | 集群：多 Agent 协作 |
| s14 Worktree | Git worktree 文件隔离 | 沙箱：并行开发 |
...
│   └── MCP (s15) ← 生态层
├── Compact (s07) ← 存活层
├── Sub-Agent (s12) → 复用 Agent Loop ← 分治层
│   └── Teams (s13) ← 协作层
├── System Prompt (s03) ← 指令层
│   ├── Memory (s08) ← 记忆层
│   ├── Settings (s06) ← 配置层
│   └── Skills (s09) + 延迟加载 ← 效率层
├── Plan Mode (s10) ← 规划层
...
**高级能力**（差异化竞争力）：
- Skills (s09) — 延迟加载 + 工具生态可扩展
- Plan Mode (s10) — 约束规划提升质量
- Tasks (s11) — 任务调度与进度追踪
- Sub-Agent (s12) — 复杂任务分治
- Teams (s13) — 多 Agent 协作
- Worktree (s14) — 文件级隔离
- MCP (s15) — 生态无限扩展
- Hooks (s05) — 企业级定制
...
这些优化不是独立存在的——它们互相增强。比如，prompt cache 的高命中率依赖于系统提示词的层级设计（s03），而层级设计又受延迟加载（s09）影响——如果所有工具描述都在系统提示词中，每次工具集变化都会打破缓存。
