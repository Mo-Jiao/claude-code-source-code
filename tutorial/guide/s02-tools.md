...
```typescript
// prompt.ts 中的延迟判定逻辑
export function shouldDeferTool(tool: Tool): boolean {
...
这个设计灵感来自 Vercel 的实验数据：**删掉 80% 的工具后，准确率从 80% 升到 100%，token 减少 40%，速度快 3.5x**。工具越少，模型越专注。
...
| 工具延迟判定 | `src/tools/ToolSearchTool/prompt.ts` | `shouldDeferTool()` 决策逻辑 |
...
然后他们做了一个激进决定——**删掉大部分工具，只保留一个 bash**。结果令人震惊：成功率 80% → **100%**，token 消耗减少 **40%**，速度提升 **3.5x** [R2-4][R2-5]。
